...
const { networkId, GAS, GUESTS_ACCOUNT_SECRET } = getConfig();
...
describe("deploy contract " + contractName, () => {
  ...
  beforeAll(async () => {
    ...
    fungibleAccount = await createOrInitAccount(
      fungibleId,
    ...
    if (fungibleAccountState.code_hash === "11111111111111111111111111111111") {
      ...
      console.log(
        "\n\n deploying fungibleAccount contractBytes:",
        fungibleContractBytes.length,
        "\n\n"
      );
      const newFungibleArgs = {
        ...
        reference_hash:
          "7c879fa7b49901d0ecc6ff5d64d7f673da5e4a5eb52a8d50a214175760d8919a",
        decimals: 24,
      };
      const actions = [
      ...
      await fungibleAccount.signAndSendTransaction({
        receiverId: fungibleId,
        actions,
      });
      /// find out how much needed to store for FTs
      storageMinimum = await contractAccount.viewFunction(
        fungibleId,
    ...
    storageMarket = await contractAccount.viewFunction(
      marketId,
      "storage_amount"
    );
  ...
  test("NFT enumerable tests (no tokens)", async () => {
    const nft_supply_for_owner = await bob.viewFunction(
      contractName,
    ...
    const bobTokens = await bob.viewFunction(
      contractName,
  ...
  test("token transfer locked - owner unlocks token transfer token type", async () => {
    ...
    const tokenLocked = await contractAccount.viewFunction(
      contractName,
      "is_token_locked",
      { token_id }
    );
    ...
    const typesLocked = await contractAccount.viewFunction(
      contractName,
  ...
  test("get sales & supply by owner id", async () => {
    const sales_by_owner_id = await contractAccount.viewFunction(
      marketId,
  ...
  test("get sales & supply by nft contract id", async () => {
    const sales_by_nft_contract_id = await contractAccount.viewFunction(
      marketId,
  ...
  test("get sales & supply by nft token type", async () => {
    const sales_by_nft_token_type = await contractAccount.viewFunction(
      marketId,
  ...
  test("bob purchase nft with NEAR", async () => {
    ...
    expect(
      new BN(aliceBalanceAfter.total)
        .sub(new BN(aliceBalanceBefore.total))
        .gt(new BN(parseNearAmount("0.79")))
    ).toEqual(true);
  });
  ...
  test("bob approves sale with FT and NEAR (fixed prices)", async () => {
    ...
    const sale = await bob.viewFunction(marketId, "get_sale", {
      nft_contract_token: contractId + DELIMETER + token_id,
    });
    console.log("\n\n get_sale", sale, "\n\n");
  ...
  test("enumerable tests", async () => {
    const total_supply = await bob.viewFunction(
      contractName,
      "nft_total_supply",
      {}
    );
    ...
    const tokens = await bob.viewFunction(contractName, "nft_tokens", {
      from_index: "0",
      limit: 100,
    });
    console.log("\n\n nft_tokens", tokens, "\n\n");
    ...
    const bobTokens = await bob.viewFunction(
      contractName,
      "nft_tokens_for_owner",
      { account_id: bobId, from_index: "0", limit: 100 }
    );
  ...
  test("alice outbids contract owner (again)", async () => {
    ...
    expect(
      new BN(contractBalanceAfter.total)
        .sub(new BN(contractBalanceBefore.total))
        .gt(new BN(parseNearAmount("1.1")))
    ).toEqual(true);
  });
  ...
  test("contract owner bids with fts", async () => {
    ...
    const marketFTBalance = await contractAccount.viewFunction(
      fungibleId,
      "ft_balance_of",
      { account_id: marketId }
    );
  ...
  test("alice outbids with FTs and bob accepts bid, alice gets NEAR back, contract owner gets FTs back", async () => {
    const token_id = tokenIds[0];
    const ownerBalanceBefore = await contractAccount.viewFunction(
      fungibleId,
      "ft_balance_of",
    ...
    /// check FT balances
    const aliceBalance = await alice.viewFunction(fungibleId, "ft_balance_of", {
      account_id: aliceId,
    });
    expect(aliceBalance).toEqual(parseNearAmount("80"));
    const marketBalance = await marketAccount.viewFunction(
      fungibleId,
      "ft_balance_of",
      { account_id: marketId }
    );
    ...
    const bobBalance = await bob.viewFunction(fungibleId, "ft_balance_of", {
      account_id: bobId,
    });
    expect(bobBalance).toEqual(parseNearAmount("16"));
    // alice's bid of 1.1 NEAR was returned (check N diff > than 1.1 - gas)
    const ownerBalanceAfter = await contractAccount.viewFunction(
      fungibleId,
      "ft_balance_of",
      { account_id: contractId }
    );
    ...
    expect(
      new BN(aliceBalanceAfter.total)
        .sub(new BN(aliceBalanceBefore.total))
        .gt(new BN(parseNearAmount("1")))
    ).toEqual(true);
    // Owner received 5% royalty in FTs on NFT purchase
    expect(
      new BN(ownerBalanceAfter).sub(new BN(ownerBalanceBefore)).toString()
    ).toEqual(parseNearAmount("11"));
  });
  ...
  test("bob fails to mint past hard cap for token type", async () => {
    ...
    const hardCap = await bob.viewFunction(contractId, "nft_supply_for_type", {
      token_type: tokenTypes[0],
    });
  ...
  test("bob: nft mints (different token_type) 2 new tokens, approve sale with NEAR open for bids", async () => {
    ...
    const sale2 = await bob.viewFunction(marketId, "get_sale", {
      nft_contract_token: contractId + DELIMETER + token_id,
    });
    console.log("\n\n", sale2, "\n\n");
  ...
  test("bob accept bid", async () => {
    ...
    expect(
      new BN(bobBalanceAfter.total)
        .sub(new BN(bobBalanceBefore.total))
        .gt(new BN(parseNearAmount("0.17")))
    ).toEqual(true);
  });

  test("get_sales_by_nft_token_type", async () => {
    const tokenTypeSales = await bob.viewFunction(
      marketId,
      "get_sales_by_nft_token_type",
      {
        token_type: tokenTypes[1],
    ...
    const tokenTypeSales2 = await bob.viewFunction(
      marketId,
      "get_sales_by_nft_token_type",
      {
        token_type: tokenTypes[2],
